...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Catering Form Submission & Success Modal
  const cateringForm = document.getElementById('cateringForm');
  const cateringSuccessModal = document.getElementById('cateringSuccessModal');
  const closeCateringModalBtn = document.getElementById('closeCateringModalBtn');
  const closeCateringModalBtn2 = document.getElementById('closeCateringModalBtn2');
  const catModalGreeting = document.getElementById('catModalGreeting');
  const catModalSummary = document.getElementById('catModalSummary');

  function closeCateringModal() {
    if (cateringSuccessModal) {
      if (typeof cateringSuccessModal.close === 'function') {
        cateringSuccessModal.close();
      } else {
        cateringSuccessModal.removeAttribute('open');
      }
    }
  }

  if (closeCateringModalBtn) closeCateringModalBtn.addEventListener('click', closeCateringModal);
  if (closeCateringModalBtn2) closeCateringModalBtn2.addEventListener('click', closeCateringModal);
  if (cateringSuccessModal) {
    cateringSuccessModal.addEventListener('click', (e) => {
      const rect = cateringSuccessModal.getBoundingClientRect();
      const isInDialog = (
        rect.top <= e.clientY &&
        e.clientY <= rect.top + rect.height &&
        rect.left <= e.clientX &&
  ...
  if (cateringForm) {
    cateringForm.addEventListener('submit', (e) => {
      ...
      const orderType = formData.get('orderType') || 'Bokningsförfrågan';
      const deliveryMethod = formData.get('deliveryMethod') || 'Avhämtning';

      // Update modal text dynamically
      if (catModalGreeting) {
        catModalGreeting.textContent = `Tack för din förfrågan, ${name}! 🎉`;
      }
      if (catModalSummary) {
        catModalSummary.innerHTML = `
          Vi har tagit emot din förfrågan gällande <strong>${orderType}</strong> för <strong>${guests}</strong> den <strong>${date}</strong> (kl. ${time}).<br><br>
          Leveranssätt: <em>${deliveryMethod}</em>.<br>
          Vi kontaktar dig på telefonnummer <strong>${phone}</strong> inom kort för att bekräfta meny och detaljer.
        `;
      }

      // Open modal dialog
      if (cateringSuccessModal) {
        if (typeof cateringSuccessModal.showModal === 'function') {
          cateringSuccessModal.showModal();
        } else {
          cateringSuccessModal.setAttribute('open', '');
        }
      }

      cateringForm.reset();
      showToast('Bokningsförfrågan skickad! Vi hörs snart.');
    });
  }
